...
static void mkcors(struct REQUEST *req) {
	if (NULL != req->cors) {
...
void mkerror(struct REQUEST *req, int status, int ka)
{
	...
	if (!ka)
		req->keep_alive = 0;
	req->lres = sprintf(req->hres, RESPONSE_START
...
void write_request(struct REQUEST *req)
{
	...
	for (;;) {
		...
		case STATE_WRITE_FILE:
			rc = wrap_xsendfile(req, req->written, req->bst.st_size - req->written);
			switch (rc) {
			case -1:
				if (errno == EAGAIN)
					return;
		...
		case STATE_WRITE_RANGES:
			if (-1 != req->rh) {
				/* write header */
				rc = wrap_write(req, req->r_head + req->rh*BR_HEADER + req->written,
			...
			if (-1 != req->rb) {
				/* write body */
				rc = wrap_xsendfile(req, req->written, req->r_end[req->rb] - req->written);
				switch (rc) {
				case -1:
					if (errno == EAGAIN)
						return;
